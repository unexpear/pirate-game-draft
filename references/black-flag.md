...
- **Deliberate deviation (we go beyond Black Flag here):** BF is essentially wind-neutral arcade handling with a simple mini-map wind arrow. We model **real points of sail** instead — a no-go zone you must tack out of, fastest on a reach, slower dead downwind, plus leeward **heel**, sails that **trim/luff**, and a masthead **wind vane HUD** showing **true + apparent wind**, the no-go zone, and **telltales**. A conscious "more realistic than BF" choice for feel and look, layered under BF's arcade *control scheme*. See [../holes.md](../holes.md).
