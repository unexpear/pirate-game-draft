...
void render(uint16_t viewId, const sea::Ship& ship, float timeSec, int width, int height) {
    ...
    // Waterline reference: a large flat translucent-looking slab at y ~ 0.
    dde.push();
    dde.setColor(abgr(30, 90, 120));
    dde.setWireframe(false);
    {
        float slab[16];
        bx::mtxSRT(slab, 60.0f, 0.02f, 60.0f, 0, 0, 0, 0.0f, 0.0f, 0.0f);
        dde.pushTransform(slab);
    ...
    // Each hull piece as a solid oriented box.
    const bx::Aabb unit = { { -0.5f, -0.5f, -0.5f }, { 0.5f, 0.5f, 0.5f } };
